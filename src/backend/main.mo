...
actor {
  ...
  public shared func loginByLegendId(
    ...
    switch (legendIdToEmail.get(legendId)) {
      case (null) {
        #err("Legend ID not found. Please double check your ID");
      };
      case (?email) {
        switch (accounts.get(email)) {
          case (null) {
            #err("Account not found for this Legend ID");
          };
          case (?accountInfo) {
            if (accountInfo.passwordHash != passwordHash) {
              #err("Wrong password. Please check your password");
            } else {
  ...
  public shared func resetPasswordByLegendId(
    legendId : Nat,
    resetCode : Text,
    newPasswordHash : Text,
  ) : async { #ok; #err : Text } {
    if (resetCode != passwordResetCode or passwordResetCode == "") {
      return #err("Invalid or expired reset code");
    };
    switch (legendIdToEmail.get(legendId)) {
      case (null) { #err("Legend ID not found") };
      case (?email) {
        switch (accounts.get(email)) {
          case (null) { #err("Account not found for this Legend ID") };
          case (?accountInfo) {
