    ...
    <div className="flex flex-col h-full">
      ...
      <div className="sticky top-0 z-10 px-3 py-3" style={{ background: "oklch(0.08 0.01 240)" }}>
        <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
          {categoriesLoading
            ? [0, 1, 2, 3].map((i) => (
                <Skeleton key={i} className="h-8 w-16 rounded-full shrink-0 bg-muted/50" />
              ))
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-4">
        {tournamentsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 gap-3 text-center">
            ...
            <p className="text-muted-foreground font-body">No tournaments found</p>
            <p className="text-xs text-muted-foreground/60">Check back soon for upcoming events</p>
          </div>
        ) : (
            ...
            {filteredTournaments.map((tournament) => {
              const slotPercent =
                tournament.totalSlots > 0n
                  ? Number((tournament.slotsFilled * 100n) / tournament.totalSlots)
                  : 0;
              const catName = categoryMap[tournament.categoryId.toString()] || "Tournament";
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    />
                    <span className="text-xs font-body text-muted-foreground">Prize:</span>
                    <span
